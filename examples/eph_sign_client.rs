...
use multi_party_eddsa::protocols::thresholdsig::{SharedKeys, Signature};
...
use crate::common::{AEAD, aes_decrypt, aes_encrypt, AES_KEY_BYTES_LEN, correct_verifiable_ss};
...
fn main() {
    ...
    let (signature, _y_sum) = run_signer(key_file_path, params, message_str);
    let sign_json = serde_json::to_string(&(
        "r",
        (BigInt::from_bytes(&(signature.R.get_element()).to_bytes())).to_str_radix(16),
        "s",
        (BigInt::from_bytes(&(signature.sigma.get_element()).to_bytes())).to_str_radix(16),
    ))
    .unwrap();

    fs::write("signature".to_string(), sign_json.clone()).expect("Unable to save !");

    println!("{:?}", sign_json);
}

fn run_signer(key_file_path: String, params: Params, message_str:String) -> (Signature, GE) {
    ///
    ...
    let (_eph_keys_vec, eph_shared_keys_vec, R, mut eph_vss_vec) = eph_keygen_t_n_parties(
        client.clone(),
        uuid.clone(),
        delay,
...
pub fn eph_keygen_t_n_parties(
    ...
    for i in 1..=n {
        if i == (party_num_int as usize) {
            ...
            let (bc1_j, blind_j, mut R_i_j) = serde_json::from_str::<(KeyGenBroadcastMessage1, BigInt, GE)>(&round1_ans_vec[j]).unwrap();
